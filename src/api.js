...
export const getAccessToken = async () => {
  try {
    ...
    if (!accessToken || tokenCheck?.error) {
      ...
      if (!code) {
        const response = await fetch(
          "https://ewnwxgc270.execute-api.us-west-1.amazonaws.com/dev/api/get-auth-url"
        );
        if (!response.ok) {
          console.error(
...
const getToken = async (code) => {
  try {
    ...
    const response = await fetch(
      "https://ewnwxgc270.execute-api.us-west-1.amazonaws.com/dev/api/token" +
        "/" +
        encodeCode
    );
    if (!response.ok) {
  ...
  try {
    ...
    // Handle when the user is online
      ...
      if (token) {
        removeQuery();
        const url =
          "https://ewnwxgc270.execute-api.us-west-1.amazonaws.com/dev/api/get-events" +
          "/" +
          token;
